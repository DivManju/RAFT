...
class Server(object):
	...
	def follower(self):
		print ('Server ID:%d running as a follower' %self.id)
		self.role = 'follower'
		self.last_update = time.time()
		election_timeout =  30 * random.random() + 5
		while time.time() - self.last_update <= election_timeout:
			pass
		self.start_election()
		while True:
			self.last_update = time.time()
			election_timeout = 1000 * random.random() + 5
			while time.time() - self.last_update <= election_timeout:
				pass
	...
	def thread_election(self):
		print ('\nTimed out! Starting new election with term %d, the candidate is %d\n' % (self.currentTerm,self.id))
		self.role = 'candidate'
		self.request_votes = self.peers[:]
		print("Candidate %d, requesting votes from my peers %s\n" %(self.id,self.peers[:]))
		sender = self.id
		...
		while 1:
			...
			for peer in self.peers:
	 			if peer in self.request_votes:
	 				...
	 				sock.sendto(data, ("127.0.0.1", self.addressbook[peer]))
			time.sleep(1) # wait for servers to receive

	def leader(self):
		print ('Server ID:%d running as a leader\n' % self.id)
		self.role = 'leader'
		self.nextIndex = {}
		self.matchIndex = {}
		for peer in self.peers:
	...
	def step_down(self):
		if self.role == 'candidate':
			print ('candidate step down when higher term found\n')
			self.election.kill()
			self.last_update = time.time()
			self.role = 'follower'
