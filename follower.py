"""Provides methods for leader election, log replication and to accept
   and process request from client
"""

__author__ = "Divya Manjunath"
__version__ = "1.0.0"
__email__ = "dm1331@scarletmail.rutgers.edu"
...
def acceptor(server, data, addr):
	...
	if _type == 'change':
		...
		if server.role != 'leader':
			print ('Redirecting config change to the leader Server ID:%d\n' %server.leaderID)
			...
			s.sendto(pickle.dumps(redirect_msg), ("127.0.0.1",server.addressbook[redirect_target]))
			s.close()			
		...
		if msg_string == 'show':
			...
			if server.role == 'leader':
				...
				if ticket_num > server.poolsize:
					print ('We are sorry tickets have sold out!')
					s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
					s.sendto('We are sorry tickets have sold out!'.encode(), addr)
					s.close()
					return
				...
				for idx, entry in enumerate(server.log):
					if entry.uuid == Msg.uuid:
						if server.commitIndex >= idx + 1:
							s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
							s.sendto('Your request has been fullfilled'.encode(), addr)
							s.close()
						else: # ignore
							pass
				...
				s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
				s.sendto('The Leader is processing  your request'.encode(), addr)
				s.close()
				server.log.append(newEntry)
				server.save()
			# we need to redirect the request to leader
			else:
				print ('Redirecting request to leader Server ID:%d\n' %(server.leaderID))
				if server.leaderID != 0:
					redirect_target = server.leaderID
				else:
					#print(server.leaderID)
					#print(server.peers)
					#print("error after line 137")
					redirect_target = random.choice(server.peers)
	...
	if _type == 1: # requestvote message
		...
		if _term < server.currentTerm:
			print ('Vote rejected by server ID:%d due to old term\n' %(server.id))
			voteGranted = 0		
		...
		if(voteGranted > 0):
			print("Vote %s granted by Server ID: %d" %(reply,server.id))
		reply_msg = VoteResponseMsg(server.id, _sender, server.currentTerm, reply)
		s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
		s.sendto(pickle.dumps(reply_msg), ("127.0.0.1",server.addressbook[_sender]))
		...
		if voteGranted:
			#server.majority =2
			#print("VoteGranted and Server during change is %s" % server.during_change)
			#print("Server Majority %s" % server.majority)
			#print("Server Role %s" % server.role)
			#print("Server Votes %s" % server.numVotes)
			if server.during_change == 0:
				if server.role == 'candidate':
					server.request_votes.remove(_sender)
					server.numVotes += 1
					if server.numVotes == server.majority:
						print ('\nWith majority of %d Server ID:%s is the leader at Term %d\n' % (server.numVotes,server.id,server.currentTerm))
					...
					if server.during_change == 0:
						...
						if compare == server.majority and server.log[N-1].term == server.currentTerm:
							for idx in range(server.commitIndex + 1, N + 1):
								...
								server.save()
								s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
								s.sendto('Your request is fullfilled'.encode(),server.log[idx-1].addr)
								s.close()
								print ('reply once')
